...
{% for number in numbers %}
{% if number > 0 %}
<li style="color:green">{{number}}</li>
{% else %}
...
{% for number in numbers %}
    <rect x="{{loop.index * 25}}" width="20" height="{{number * 3}}" style="fill:rgb(0,206,209);" />
{% endfor %}
</svg>
...
{% for number in numbers %}
    <rect y="{{loop.index * 25}}" width="{{number * 3}}" height="20" style="fill:rgb(0,206,209);" />
{% endfor %}
</svg>
...
{% for point in data %}
    <circle cx="{{point[0] * 12}}" cy="{{point[1] * 3}}" r="3" fill="black" />
{% endfor %}
</svg>
...
<svg width="500" height="200" style="border:1px solid grey">
{% for point in data %}
    <circle cx="{{point[0] * 12}}" cy="{{point[1] * 3}}" r="{{point[2] * 2}}" fill="red" />
{% endfor %}
</svg>
